...
with tab2:
    ...
    st.markdown(
        "This project uses the **LMSYS Chatbot Arena** dataset to build machine learning models "
        "that predict which LLM response humans prefer — Model A, Model B, or a tie — "
        "using response-length features, prompt characteristics, model identity, and "
        "50-dimensional semantic embeddings of each prompt.\n\n"
        f"Our best model achieved an F1 of **{best_f1:.4f}**, and SHAP analysis reveals that "
        "length-based features consistently dominate human preference judgments."
    )
    hr()
